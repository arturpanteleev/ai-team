...
- **Схема approval-записи поднята до версии 2** (breaking): добавлено
  обязательное поле `integrity`. Approvals, сохранённые прежними версиями, при
  продолжении run отклоняются с явной причиной — такое решение нужно запросить
  заново; записи новой схемы не читаются бинарником старой версии (#152).

### Fixed

- **Approval-запись больше не подделывается правкой файла (QS-01, #152).**
  Решения в `.ai-team/state/approvals/` были обычным JSON без подписи, поэтому
  LLM-этап, имеющий доступ к файловой системе target, мог перевести
  delivery-approval в `resolved`/`approve` и получить commit/push/PR при
  `--resume` — без `--approve-plan`, без `ai-team decision` и без TTY. Теперь
  каждая запись аутентифицирована HMAC-SHA256 на ключе контроллера, который
  хранится вне target (`<user-config-dir>/ai-team/secrets/approval.key`, `0400`;
  путь переопределяется `AI_TEAM_APPROVAL_KEY`), и MAC проверяется до
  применения семантики записи — изменённое на диске или созданное мимо
  контроллера решение отклоняется fail-closed. Легитимные пути
  (`ai-team decision`, `--approve-plan <sha256>`, web UI) не изменились.
