...
### Issue и OpenSpec change: граница

Работа **планируется** в GitHub Issues, а **проектируется** в
`openspec/changes/`. Чтобы два реестра не разъехались, между ними проведена
жёсткая граница.

| Живёт в issue | Живёт в change |
|---|---|
| Симптом и боль, 2–4 предложения на языке пользователя | Нормативное поведение (`specs/`) |
| Почему это важно сейчас — приоритет | Как именно, с рассмотренными альтернативами (`design.md`) |
| Ссылка на `openspec/changes/<name>/` | Шаги реализации (`tasks.md`) |
| Координация: кто взял, обсуждение, ссылки на PR | Критерий приёмки и Impact |

Инвариант, который делает противоречие невозможным: **issue не повторяет
change, а ссылается на него.** Нормативные формулировки, критерии приёмки и
шаги реализации существуют ровно в одном месте — в change. То, что нигде не
продублировано, не может разойтись.

Отсюда три практических правила:

1. **Change всегда прав.** Если текст issue и текст change расходятся, верен
   change. Обратное не выполняется никогда: issue — указатель, а не источник.
2. **Уточнение постановки идёт в change, а не в issue.** Если по ходу
   обсуждения формулировка изменилась, правится `proposal.md`; issue остаётся
   коротким указателем и не переписывается вслед.
3. **Один issue — один change.** Не заводите issue на каждый пункт
   `tasks.md`: это создаёт второй реестр задач, который придётся
   синхронизировать руками. Декомпозиция живёт внутри change.

#### Жизненный цикл

Change создаётся **не при заведении issue, а когда задача берётся в работу**.
Issue может долго лежать в бэклоге без change — это нормальное состояние, а не
недоделка: постановка задачи и её проектирование разнесены во времени
намеренно.

1. **Заведение.** Issue описывает проблему; поле «OpenSpec change» пустое,
   метка `needs-change`. На этом этапе issue обсуждается и приоритизируется.
   Change не создаётся.
2. **Взятие в работу.** Исполнитель ставит себя в assignee и проходит цикл
   Explore → Propose → Design → Specs → Tasks. Как только появился
   `proposal.md`, путь `openspec/changes/<name>/` проставляется в issue, а
   метка `needs-change` снимается.
3. **Реализация.** К моменту коммита и push change обязан существовать:
   PR с поведенческим изменением без change не принимается.
4. **PR.** Строка `Closes #<номер>` закрывает issue на merge.
5. **Archive.** От issue ничего не требует: он закрыт ещё на merge.

Изменения без наблюдаемого поведения (опечатки, тесты на уже
специфицированное поведение, рефакторинг, документация) change не требуют —
такой issue помечается `no-change`, и в PR отмечается соответствующий пункт
шаблона.

Соответствие сейчас проверяет ревьюер глазами. Автоматической проверки нет
намеренно: часть правок change не требует, и жёсткий гейт ломал бы их, а на
внешних контрибьюторах работал бы как барьер. Механическая проверка
запланирована отдельной задачей — PDD-27 в
[`docs/PRODUCT-REVIEW-2026-09.md`](docs/PRODUCT-REVIEW-2026-09.md).
