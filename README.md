...
Каждая такая запись аутентифицирована HMAC-SHA256 на ключе контроллера, и
проверяется этот MAC раньше, чем применяется смысл записи: approval,
изменённый на диске после записи контроллером, и approval, созданный мимо
него, отклоняются fail-closed — resume в обоих случаях останавливается до
delivery. Ключ генерируется при первом обращении в
`<user-config-dir>/ai-team/secrets/approval.key` (macOS: `~/Library/Application
Support/…`, Linux: `~/.config/…`) с правами `0400`; каталог — `0700`. Путь
переопределяется переменной `AI_TEAM_APPROVAL_KEY`, но **внутри target** ключ
лежать не может: туда пишет агент, и ai-team откажется стартовать. В окружение
агент-субпроцесса эта переменная не попадает (allow-list, см.
[«Runtime и credentials»](#runtime-и-credentials)). Потеря ключа не ломает
проект, но делает уже сохранённые approvals нечитаемыми — решения придётся
запросить заново.
...
То же ограничение определяет и силу MAC на approval-записях. Ключ вынесен за
пределы target — туда, куда агент по условиям задачи не пишет, и куда не
доезжает его окружение, — так что подделать решение человека правкой файлов в
проекте нельзя. Но агент исполняется под тем же OS-user, и пока нет
OS-enforced sandbox, недоверенный код, вырвавшийся за app-level ограничения,
может добраться и до файла ключа. Fail-closed проверка отличит подделку от
решения контроллера; она не заменяет изоляцию процесса.
