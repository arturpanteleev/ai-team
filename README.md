...
**`delivery_timeout`** (необязательное, по умолчанию `10m`) задаёт бюджет всей
post-terminal доставки — `git commit`, `git push`, `gh pr view/create` вместе.
Доставка одобрена человеком и исполняется уже после terminal finalize, поэтому
она сознательно **не** ограничена `budget.max_wall_time` run'а: исчерпанный
бюджет запуска её не обрывает. Но её команды сетевые, и без собственного
дедлайна зависший `git` или `gh` держал бы контроллер бесконечно, поэтому
дедлайн у доставки свой. Прерывается она двумя способами: по этому бюджету и по
сигналу процессу (`Ctrl-C`). Дефолт щедрый — доставка до живого remote
измеряется секундами; поднимайте значение для медленной сети или очень
больших push.

Внешние команды доставки при этом исполняются неинтерактивно
(`GIT_TERMINAL_PROMPT=0`, `GH_PROMPT_DISABLED=1`, askpass-хуки не наследуются):
у контроллера нет терминала, и запрос пароля стал бы зависанием, а не вопросом.
Если доступ к remote требует явной настройки SSH, задайте `GIT_SSH_COMMAND` —
своё значение доставка уважает.

```yaml
delivery_timeout: 10m
```
