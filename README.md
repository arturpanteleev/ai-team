...
| `ai-team usage [--target <dir>] <run_id>` | usage-сводка завершённого run: этапы, попытки, время |
...
| `ai-team gc [--target <dir>] [--older-than 720h] [--keep-last 20] [--dry-run] [--prune-runs]` | уборка растущих артефактов `.ai-team`; `--prune-runs` удаляет immutable evidence только у run с verified-экспортом |
...
**`preflight_timeout`** (необязательное, по умолчанию `60s`) задаёт бюджет
одной внешней команды preflight — опроса версии CLI-рантайма, вызовов `git` и
`gh`. Превышение бюджета по-прежнему даёт failed required check и не допускает
запуск; настраивается именно величина. Значение имеет смысл поднять, если
рантайм стартует долго: холодный старт `opencode` занимает секунды против долей
секунды у прогретого, и слишком тесный бюджет делает работоспособный рантайм
неотличимым от отсутствующего.

```yaml
preflight_timeout: 60s
```

### Лимиты времени и попыток

Run ограничен всегда, даже если в конфиге нет ни одного из полей ниже:
дефолты применяются на уровне кода, а не только в том, что записал `init`.

| Поле | По умолчанию | Что ограничивает |
|---|---|---|
| `budget.max_wall_time` | `24h` | суммарное wall-time всего run'а, включая ожидание человека на approval-гейтах |
| `budget.max_attempts` | `100` | суммарное число попыток стадий за run |
| `stage_timeout` | `30m` | бюджет одной стадии; `init` пишет это значение явно |
| `timeout` (у агента в `pipeline`) | значение `stage_timeout` | бюджет конкретной стадии, перекрывает глобальный |
...
Секция `budget` необязательна: без неё действуют те же `24h` и `100`.
Дефолт wall-time выбран так, чтобы переживать ожидание человека — run целиком,
вместе с approval-гейтами, исполняется под этим лимитом, поэтому «запустил
вечером, подтвердил утром» не должно упираться в бюджет. Понижайте значение,
если запускаете run без человека в цикле (CI, batch): тогда верхняя граница
имеет смысл в часах, а не в сутках.

Превышение любого из лимитов останавливает run с причиной, называющей
конкретный лимит (`run budget: превышен max_wall_time 24h`,
`run budget: превышен лимит попыток max_attempts=100`). Превышение
`stage_timeout` — это таймаут **стадии**, а не терминальный бюджет run'а: run
остаётся resumable через `ai-team run --resume <run_id>`.

Значения парсятся как Go-длительности (`30m`, `2h`, `90s`); непарсящееся или
неположительное значение отвергается валидацией конфига до первого LLM-вызова.
