...
В bundle gate пишет **risk-signals** (V0-8): размер diff в строках и файлах,
количество тестовых изменений, чувствительные пути (.env, ключи, credentials)
и число failed checks. Это данные для будущего adaptive routing и мониторинга;
пока записываются строкой в `gate summary` и в bundle — маршрутизацию не
меняют, автоматическое решение по ним не принимается.
