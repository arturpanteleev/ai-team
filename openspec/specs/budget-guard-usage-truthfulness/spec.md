# budget-guard-usage-truthfulness Specification

## Purpose
Жёсткие бюджеты run'а (wall-time и число попыток, канонические дефолты без секции конфига) и правдивость usage: tokens/cost принимаются только от attested adapter-источника.
## Requirements
