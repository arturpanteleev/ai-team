...
GET под `/api/` без совпавшего роута MUST получать 404 с `Content-Type: application/json` и объясняющим телом, а промах по методу MUST оставаться ответом 405 с заголовком `Allow`; ни один из них MUST NOT отдавать HTML.

#### Scenario: Неизвестный API-путь

- **КОГДА** клиент делает GET по `/api/` пути без зарегистрированного роута,
  включая корень `/api`
...
- **КОГДА** метод запроса не зарегистрирован ни для одного роута существующего
  `/api/` пути
- **ТОГДА** сервер MUST вернуть 405 с заголовком `Allow`, перечисляющим
  реально доступные методы
