...
### Requirement: Machine-readable API errors

Промах мимо роутов под `/api/` MUST отвечать JSON: 404, если путь не обслуживается ни одним методом, и 405 с заголовком `Allow`, если не поддержан только метод; ответ MUST NOT зависеть от того, собран ли фронтенд.

#### Scenario: Неизвестный API-путь

- **КОГДА** клиент любым методом запрашивает `/api/` путь, который не
  обслуживается ни одним зарегистрированным роутом, включая корень `/api`
  и пути, чья percent-encoded форма не совпадает ни с одним роутом
- **ТОГДА** сервер MUST вернуть 404 с JSON-телом
- **И** MUST NOT отдавать HTML страницы дашборда
- **И** MUST NOT выставлять `Allow`, поскольку обслуживаемых методов нет

#### Scenario: Неподдерживаемый метод

- **КОГДА** метод не зарегистрирован для существующего `/api/` пути
- **ТОГДА** сервер MUST вернуть 405 с JSON-телом
- **И** `Allow` MUST перечислять только методы, которые этот путь
  действительно обслуживает, без SPA-fallback

#### Scenario: Маршрут клиентского роутера

- **КОГДА** собран фронтенд и методом GET запрошен путь вне `/api/`,
  которому не соответствует файл дистрибутива
- **ТОГДА** сервер MUST отдать SPA index для клиентского роутинга
