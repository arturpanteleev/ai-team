...
Любой путь под `/api/` без зарегистрированного роута MUST получать 404 с `Content-Type: application/json` и объясняющим телом, а SPA-fallback MUST действовать только вне `/api/`.

#### Scenario: Неизвестный API-путь

- **КОГДА** клиент запрашивает `/api/` путь без зарегистрированного роута
- **ТОГДА** сервер MUST вернуть 404 с JSON-телом
- **И** MUST NOT отдавать HTML страницы дашборда

#### Scenario: Неподдерживаемый метод

- **КОГДА** метод запроса не зарегистрирован для существующего `/api/` пути
- **ТОГДА** сервер MUST вернуть тот же JSON-ответ 404, а не HTML

#### Scenario: Маршрут клиентского роутера

- **КОГДА** запрошен путь вне `/api/`, которому не соответствует файл фронтенда
