...
Frontend ДОЛЖЕН отображать детали pipeline run.

#### Scenario: Отображение этапов
- **КОГДА** пользователь открывает `/pipelines/:id`
- **ТОГДА** страница ДОЛЖНА показать:
...
- **ТОГДА** страница ДОЛЖНА обновить статус этапа без перезагрузки

#### Scenario: Артефакты этапа
- **КОГДА** пользователь раскрывает этап
- **ТОГДА** страница ДОЛЖНА показать список артефактов (inputs/outputs)
- **И** каждый артефакт — кликабельная ссылка на Artifact Viewer

### Requirement: Status badges
Frontend ДОЛЖЕН отображать цветовые badge для статусов.

#### Scenario: Цвета статусов
- **КОГДА** этап имеет статус
- **ТОГДА** badge ДОЛЖЕН быть:
  - running — синий
  - completed — зелёный
  - failed — красный
  - blocked — оранжевый